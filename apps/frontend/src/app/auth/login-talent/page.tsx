  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mergedUser = {
        ...
        skillBadgeId: data.user.skillBadgeId, // IMPORTANT: sauvegarder l'ID SkillBadge
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mergedUser = {
          ...
          email: data.user.email,
          role: data.user.role,
          fullName: user.displayName || data.user.fullName,
          skillBadgeId: data.user.skillBadgeId,
          walletAddress: data.user.walletAddress
        }
        ...
        const errorData = await response.json()
        throw new Error(errorData.message || 'Erreur avec Google Auth')
      }
    } catch (err: any) {
      ...
      if (err.code === 'auth/popup-blocked') {
        setError('Le popup a été bloqué par votre navigateur. Autorisez les popups pour ce site.')
      } else if (err.code === 'auth/popup-closed-by-user') {
        setError('Connexion annulée')
      } else if (err.code === 'auth/cancelled-popup-request') {
        // Popup interrompu - rafraîchir la page
        setError('Connexion interrompue. Rafraîchissez la page et réessayez.')
      } else if (err.code === 'auth/unauthorized-domain') {
        setError('Erreur de configuration Google Auth. Contactez le support.')
      } else {
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="p-6 sm:p-8 flex flex-col justify-center">
            ...
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              {/* Email */}
              <div className="relative">
                <label className="absolute left-4 -top-2.5 bg-white px-2 text-sm font-bold text-gray-700 uppercase tracking-wider">
                  EMAIL
                </label>
                <input 
                  type="email"
                  ...
                  required
                  className="w-full h-14 rounded-lg px-5 text-lg font-medium placeholder-gray-500 outline-none focus:ring-2 focus:ring-orange-500/50 transition-all border-2 border-gray-300"
